...
# Crear los widgets de texto y botones
text_widget = tk.Text(root, height=10, width=30)
text_widget.pack(side=tk.LEFT)
button_start = tk.Button(root, text="COMIENZA", command=lambda: cap.open('C:\\Users\\User\\Videos\\camaron2.mp4'))
button_start.pack()
button_stop = tk.Button(root, text="DETENGA", command=lambda: cap.release())
button_stop.pack()
button_exit = tk.Button(root, text="SALIR", command=lambda: root.destroy())
button_exit.pack()
...
while True:
    ...
    for result in results.xyxy[0]:
        ...
        cv2.putText(section_frame, section_label, (section_x_pos, section_y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
